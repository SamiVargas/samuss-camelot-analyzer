...
async function decodeAudioWindow(file) {
  ...
  const middleSample = Math.floor(audioBuffer.length / 2);
  const startSample = Math.max(0, Math.min(middleSample - Math.floor(requestedSamples / 2), audioBuffer.length - requestedSamples));
  const endSample = Math.min(audioBuffer.length, startSample + requestedSamples);
  const actualSamples = endSample - startSample;
  const monoSamples = new Float32Array(actualSamples);

  for (let channel = 0; channel < audioBuffer.numberOfChannels; channel += 1) {
    const channelSamples = audioBuffer.getChannelData(channel).subarray(startSample, endSample);
...
function estimateBpm(samples, sampleRate) {
  ...
  for (let index = 1; index < envelope.length - 1; index += 1) {
    if (envelope[index] > envelope[index - 1] && envelope[index] > envelope[index + 1] && envelope[index] > averageEnergy * 1.25) {
...
function correlate(chroma, profile) {
  ...
  return chroma.reduce((sum, value, index) => sum + (value - chromaMean) * (profile[index] - profileMean), 0);
}
...
function sanitizeFileName(fileName) {
  return fileName.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[<>:"/\\|?*\x00-\x1F]/g, "").replace(/\s+/g, " ").trim();
}
...
async function analyzeTrack(track) {
  ...
  track.status = "Analizado desde la mitad";
  track.progress = 100;
  renderAll();
}
...
function downloadPlaylistCsv() {
  ...
  const rows = state.orderedPlaylist.map((item, index) => [index + 1, item.track.originalName, item.track.bpm || "", item.track.camelot, item.transition]);
...
function renderStats() {
  ...
  const averageBpm = tracksWithBpm.length ? Math.round(tracksWithBpm.reduce((sum, track) => sum + track.bpm, 0) / tracksWithBpm.length) : "--";
