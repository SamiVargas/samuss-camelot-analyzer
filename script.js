...
/**
 * Calcula magnitud para una frecuencia objetivo mediante DFT puntual.
 */
function getFrequencyMagnitude(samples, sampleRate, frequency) {
  const step = Math.max(1, Math.floor(samples.length / 8192));
  ...
  for (let sampleIndex = 0; sampleIndex < samples.length; sampleIndex += step) {
    ...
    real += sample * Math.cos(angularFrequency * sampleIndex);
    imaginary -= sample * Math.sin(angularFrequency * sampleIndex);
  }
...
/**
 * Construye un vector cromatico de 12 clases de tono.
 */
function buildChromaVector(samples, sampleRate) {
  ...
  for (let midi = 36; midi <= 84; midi += 1) {
    ...
    chroma[midi % 12] += getFrequencyMagnitude(samples, sampleRate, frequency);
  }
...
function detectMusicalKey(samples, sampleRate) {
  ...
  return {
    keyName: `${NOTE_NAMES[bestResult.tonicIndex]} ${bestResult.mode}`,
